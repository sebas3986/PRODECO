...
type
  TFrm_Recepcion = class(TForm)
    ...
    procedure btnPreimpresoPlaClick(Sender: TObject);
    procedure edPreimpresoPlaChange(Sender: TObject);
  private
...
begin
  ...
  if edPreimpresoCaj.Text = '' then
  begin
    ShowMessage('El código preimpreso de la caja no puede estar vacio, verifique.');
    edPreimpresoCaj.SetFocus;
    Exit;
  end
  else
  begin
    if DMRecepcion.BuscarCodPreimpreso(edPreimpresoCaj.Text,2) = True then
    begin
      ShowMessage('Ya se encuentra asignado el código preimpreso a una caja ['+edPreimpresoCaj.Text+'], verifique.');
      edPreimpresoCaj.SetFocus;
      Exit;
    end;
  end;
...
procedure TFrm_Recepcion.btnPreimpresoPlaClick(Sender: TObject);
begin
...
begin
  DMRecepcion.CerrarCaja(DMRecepcion.cdsCaja.FieldByName('idCaja').AsInteger,edPreimpresoCaj.Text);
  ...
  edTotalCaja.Text := '';
  //Sebastian Camacho 28/09/2018
  edPreimpresoCaj.Text := '';
  edPreimpresoCaj.Enabled := False;
  btnCarpeta.Visible := False;
               ...
               if NOT DMRecepcion.cdsCaja.FieldByName('Cerrada').AsBoolean then  //Verifica si la caja esta cerrada, de no ser así procede al cargue de Carpetas
                 begin
                  ...
                  begin
                    ...
                    edCaja.Text := DMRecepcion.cdsCaja.FieldByName('CodigoCaja').AsString;
                    //Sebastian Camacho 28/09/2018
                    edPreimpresoCaj.text := DMRecepcion.cdsCaja.FieldByName('codpreimpreso').AsString;
                    edPrecinto1.Text :=  DMRecepcion.cdsCaja.FieldByName('PrecintoUno').AsString;
                    edPrecinto2.Text :=  DMRecepcion.cdsCaja.FieldByName('PrecintoDos').AsString;
                    edPrecinto1.Enabled := False;
...
procedure TFrm_Recepcion.edPreimpresoPlaChange(Sender: TObject);
begin
