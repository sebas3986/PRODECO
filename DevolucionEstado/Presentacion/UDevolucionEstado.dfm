object frmDevolucionEstado: TfrmDevolucionEstado
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Carpeta Digital - [ Versi'#243'n  ]'
  ClientHeight = 752
  ClientWidth = 1283
  Color = clWhite
  DefaultMonitor = dmDesktop
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clBlack
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  KeyPreview = True
  OldCreateOrder = False
  Position = poDesktopCenter
  Visible = True
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object PnlTitulo: TPanel
    Left = 0
    Top = 0
    Width = 1283
    Height = 73
    Align = alTop
    Caption = 'DEVOLUCION DE ESTADOS PARA CARPETAS'
    Color = 15259846
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlack
    Font.Height = -24
    Font.Name = 'Tahoma'
    Font.Style = [fsBold]
    ParentBackground = False
    ParentFont = False
    TabOrder = 0
  end
  object stbBarraEstado: TStatusBar
    Left = 0
    Top = 726
    Width = 1283
    Height = 26
    Panels = <
      item
        Bevel = pbRaised
        BiDiMode = bdLeftToRight
        ParentBiDiMode = False
        Width = 400
      end
      item
        Bevel = pbRaised
        BiDiMode = bdLeftToRight
        ParentBiDiMode = False
        Width = 150
      end>
  end
  object GrpPie: TGroupBox
    Left = 0
    Top = 673
    Width = 1283
    Height = 53
    Align = alBottom
    Color = 14995646
    ParentBackground = False
    ParentColor = False
    TabOrder = 2
    DesignSize = (
      1283
      53)
    object spbSalir: TSpeedButton
      Left = 1137
      Top = 5
      Width = 113
      Height = 45
      Caption = '&Salir'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      OnClick = spbSalirClick
    end
    object RpnMensajeProceso: TRxPanel
      Left = 617
      Top = 6
      Width = 396
      Height = 41
      Anchors = []
      BevelInner = bvRaised
      BevelKind = bkFlat
      BevelOuter = bvNone
      Caption = 'DEVOLVIENDO CARPETA ....'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clMaroon
      Font.Height = -27
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 2
      Visible = False
      Gradient.Direction = fdTopToBottom
      Gradient.EndColor = clSilver
      Gradient.StartColor = 16754856
      Gradient.Visible = True
      TileImage = False
    end
    object bbtOtraCaja: TBitBtn
      Left = 18
      Top = 6
      Width = 218
      Height = 41
      Caption = 'Seleccionar Otra Caja'
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        7055557FFFFFF777F7F555000000000000555577777777777755}
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 0
      OnClick = bbtOtraCajaClick
    end
    object bbtEfectuarDevolucion: TBitBtn
      Left = 611
      Top = 6
      Width = 218
      Height = 41
      Caption = 'Efectuar Devoluci'#243'n'
      Enabled = False
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFF}
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 1
      OnClick = bbtEfectuarDevolucionClick
    end
    object bbtOtraCarpeta: TBitBtn
      Left = 258
      Top = 6
      Width = 218
      Height = 41
      Caption = 'Seleccionar Otra Carpeta'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = []
      Glyph.Data = {
        
        5555557777555555555555555555555555555555555555555555}
      NumGlyphs = 2
      ParentFont = False
      TabOrder = 3
      Visible = False
      OnClick = bbtOtraCarpetaClick
    end
  end
  object RpnCaptura: TRxPanel
    Left = 0
    Top = 73
    Width = 1283
    Height = 600
    Align = alClient
    BevelOuter = bvNone
    TabOrder = 1
    Gradient.Direction = fdBottomToTop
    Gradient.EndColor = 12885363
    Gradient.StartColor = 16315117
    Gradient.Visible = True
    TileImage = False
    object RpnCriteriosBusqueda: TRxPanel
      Left = 0
      Top = 0
      Width = 1283
      Height = 600
      Align = alClient
      BevelOuter = bvNone
      Color = 15655895
      ParentBackground = False
      TabOrder = 0
      Gradient.Direction = fdBottomToTop
      Gradient.EndColor = 12885363
      Gradient.StartColor = 15786969
      Gradient.Visible = True
      TileImage = False
      object GrpSeleccionCarpeta: TGroupBox
        Left = 0
        Top = 0
        Width = 585
        Height = 600
        Align = alLeft
        Caption = 'Selecci'#243'n de la Carpeta'
        DoubleBuffered = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -15
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentDoubleBuffered = False
        ParentFont = False
        TabOrder = 0
        DesignSize = (
          585
          600)
        object PnlCriteriosAportes: TPanel
          Left = 18
          Top = 338
          Width = 550
          Height = 249
          BevelInner = bvLowered
          TabOrder = 0
          object LblDatosCarpeta: TLabel
            Left = 18
            Top = 10
            Width = 300
            Height = 22
            Alignment = taCenter
            Caption = 'INFORMACI'#211'N DE LA CARPETA'
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -19
            Font.Name = 'Arial Rounded MT Bold'
            Font.Style = []
            ParentFont = False
          end
          object stgDatosCarpeta: TStringGrid
            Left = 18
            Top = 38
            Width = 511
            Height = 204
            Color = clMenuHighlight
            ColCount = 2
            Ctl3D = True
            DoubleBuffered = False
            DrawingStyle = gdsClassic
            Enabled = False
            FixedColor = clGradientActiveCaption
            FixedCols = 0
            RowCount = 8
            FixedRows = 0
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -16
            Font.Name = 'Arial Narrow'
            Font.Style = [fsBold]
            GradientEndColor = 4227327
            GradientStartColor = clBlue
            Options = [goVertLine, goHorzLine, goFixedHotTrack]
            ParentCtl3D = False
            ParentDoubleBuffered = False
            ParentFont = False
            TabOrder = 0
            OnDrawCell = stgDatosCarpetaDrawCell
          end
        end
        object PnlListaCarpetas: TPanel
          Left = 18
          Top = 127
          Width = 550
          Height = 197
          Anchors = [akLeft, akTop, akRight, akBottom]
          BevelInner = bvLowered
          TabOrder = 1
          DesignSize = (
            550
            197)
          object LblListaCarpetas: TLabel
            Left = 18
            Top = 14
            Width = 227
            Height = 22
            Alignment = taCenter
            Caption = 'CARPETAS DE LA CAJA'
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -19
            Font.Name = 'Arial Rounded MT Bold'
            Font.Style = []
            ParentFont = False
          end
          object LblAyudaCarpetas: TLabel
            Left = 286
            Top = 110
            Width = 243
            Height = 65
            Margins.Left = 15
            Alignment = taCenter
            Anchors = [akLeft, akRight]
            AutoSize = False
            Caption = 
              'Seleccione con Doble-Click '#13#10'la Carpeta que desea '#13#10'devolver de ' +
              'Estado'
            Color = clMoneyGreen
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -15
            Font.Name = 'Arial'
            Font.Style = [fsItalic]
            ParentColor = False
            ParentFont = False
            Transparent = False
            Layout = tlCenter
            WordWrap = True
          end
          object LblCantidadCarpetas: TLabel
            Left = 286
            Top = 51
            Width = 243
            Height = 42
            Margins.Left = 15
            Alignment = taCenter
            Anchors = [akLeft, akRight]
            AutoSize = False
            Caption = 'CANTIDAD CARPETAS'
            Color = clMenuHighlight
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -15
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentColor = False
            ParentFont = False
            Transparent = False
            Layout = tlCenter
            WordWrap = True
          end
          object dbgListaCarpetas: TDBGrid
            Left = 18
            Top = 42
            Width = 250
            Height = 142
            Color = clGradientActiveCaption
            DrawingStyle = gdsClassic
            Enabled = False
            FixedColor = 8453888
            GradientEndColor = clBlack
            Options = [dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleHotTrack]
            TabOrder = 0
            TitleFont.Charset = DEFAULT_CHARSET
            TitleFont.Color = clBlack
            TitleFont.Height = -15
            TitleFont.Name = 'Tahoma'
            TitleFont.Style = []
            OnDblClick = dbgListaCarpetasDblClick
            Columns = <
              item
                Color = 16760445
                Expanded = False
                FieldName = 'codigocarpeta'
                Width = 200
                Visible = True
              end>
          end
        end
        object PnlCaja: TPanel
          Left = 2
          Top = 20
          Width = 581
          Height = 103
          Align = alTop
          BevelEdges = []
          BevelOuter = bvNone
          Color = 15259846
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -24
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentFont = False
          TabOrder = 2
          DesignSize = (
            581
            103)
          object LblEme: TLabel
            Left = 184
            Top = 28
            Width = 19
            Height = 25
            Caption = 'M'
            Color = clBlue
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clNavy
            Font.Height = -21
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            ParentColor = False
            ParentFont = False
          end
          object LblCodigoCaja: TLabel
            Left = 21
            Top = 28
            Width = 153
            Height = 23
            Caption = 'C'#243'digo de la Caja:'
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -19
            Font.Name = 'Tahoma'
            Font.Style = []
            ParentFont = False
          end
          object LblMensajeError: TLabel
            Left = 183
            Top = 63
            Width = 381
            Height = 38
            Margins.Left = 15
            Alignment = taCenter
            Anchors = [akLeft, akRight]
            AutoSize = False
            Caption = 'ESTA CAJA NO POSEE CARPETAS ASOCIADAS'
            Color = 1185180
            Font.Charset = ANSI_CHARSET
            Font.Color = clWhite
            Font.Height = -15
            Font.Name = 'Arial Rounded MT Bold'
            Font.Style = [fsBold]
            ParentColor = False
            ParentFont = False
            Transparent = False
            Layout = tlCenter
            Visible = False
            WordWrap = True
          end
          object spbConsultarCarpeta: TSpeedButton
            Left = 358
            Top = 20
            Width = 207
            Height = 38
            Caption = 'Consultar Car&petas de la Caja'
            Enabled = False
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clWindowText
            Font.Height = -12
            Font.Name = 'Tahoma'
            Font.Style = []
            Glyph.Data = {
              
              0000}
            NumGlyphs = 2
            ParentFont = False
            Spacing = 3
            Transparent = False
            OnClick = spbConsultarCarpetaClick
          end
          object EdtCodigoCaja: TEdit
            Left = 206
            Top = 26
            Width = 76
            Height = 30
            Color = 50319311
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -19
            Font.Name = 'Arial Rounded MT Bold'
            Font.Style = []
            MaxLength = 4
            ParentFont = False
            TabOrder = 0
            OnChange = EdtCodigoCajaChange
            OnKeyPress = EdtCodigoCajaKeyPress
          end
        end
      end
      object GrpSeleccionEstado: TGroupBox
        Left = 585
        Top = 0
        Width = 698
        Height = 600
        Align = alClient
        BiDiMode = bdLeftToRight
        Caption = 'Selecci'#243'n Estado '
        Enabled = False
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -16
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentBiDiMode = False
        ParentFont = False
        TabOrder = 1
        DesignSize = (
          698
          600)
        object LblEstadoDevolver: TLabel
          Left = 24
          Top = 39
          Width = 338
          Height = 23
          Caption = 'Estado al cual desea devolver la Carpeta'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
        end
        object dblEstadoDevolver: TDBLookupComboBox
          Left = 383
          Top = 38
          Width = 282
          Height = 26
          Color = 9882571
          DropDownRows = 4
          Font.Charset = ANSI_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          ParentShowHint = False
          ShowHint = False
          TabOrder = 0
          OnCloseUp = dblEstadoDevolverCloseUp
          OnEnter = dblEstadoDevolverEnter
        end
        object GrpCriteriosAdicionales: TGroupBox
          Left = 24
          Top = 96
          Width = 651
          Height = 108
          Anchors = [akLeft, akTop, akRight]
          Caption = 'Criterios Adicionales '
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -15
          Font.Name = 'Tahoma'
          Font.Style = []
          ParentFont = False
          TabOrder = 1
          DesignSize = (
            651
            108)
          object Lbl1: TLabel
            Left = 224
            Top = 38
            Width = 408
            Height = 47
            Margins.Left = 15
            Alignment = taCenter
            Anchors = [akLeft, akRight]
            AutoSize = False
            Caption = 
              'Se elimina el C'#243'digo de la Carpeta dentro de la Caja, para que  ' +
              'se radique nuevamente.'
            Color = clMoneyGreen
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -15
            Font.Name = 'Arial'
            Font.Style = [fsItalic]
            ParentColor = False
            ParentFont = False
            Transparent = False
            Layout = tlCenter
            WordWrap = True
            ExplicitTop = 40
            ExplicitWidth = 398
          end
          object chkEliminaCarpeta: TCheckBox
            Left = 23
            Top = 45
            Width = 198
            Height = 34
            Caption = 'Elimina la Carpeta'
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -16
            Font.Name = 'Arial Narrow'
            Font.Style = [fsBold]
            ParentFont = False
            TabOrder = 0
            WordWrap = True
          end
        end
        object PnlEstadoProceso: TPanel
          Left = 24
          Top = 226
          Width = 651
          Height = 363
          Anchors = [akLeft, akTop, akRight, akBottom]
          BevelInner = bvLowered
          TabOrder = 2
          object LblDevolverCarpeta: TLabel
            Left = 2
            Top = 49
            Width = 647
            Height = 41
            Margins.Left = 15
            Align = alTop
            Alignment = taCenter
            AutoSize = False
            Caption = 'CARPETA EN PROCESO'
            Color = 13478273
            Font.Charset = ANSI_CHARSET
            Font.Color = 1185180
            Font.Height = -16
            Font.Name = 'Arial'
            Font.Style = [fsBold]
            ParentColor = False
            ParentFont = False
            Transparent = False
            Layout = tlCenter
            Visible = False
            WordWrap = True
            ExplicitTop = 43
            ExplicitWidth = 637
          end
          object PnlTituloEstado: TPanel
            Left = 2
            Top = 2
            Width = 647
            Height = 47
            Align = alTop
            Caption = 'Estado del Proceso'
            Color = 15259846
            Font.Charset = DEFAULT_CHARSET
            Font.Color = clBlack
            Font.Height = -24
            Font.Name = 'Tahoma'
            Font.Style = [fsBold]
            ParentBackground = False
            ParentFont = False
            TabOrder = 0
          end
          object stgEstadoProceso: TStringGrid
            Left = 2
            Top = 90
            Width = 647
            Height = 271
            Align = alClient
            BevelOuter = bvSpace
            Color = clWhite
            ColCount = 2
            DoubleBuffered = False
            DrawingStyle = gdsClassic
            Enabled = False
            FixedColor = clWhite
            FixedCols = 0
            RowCount = 10
            FixedRows = 0
            Font.Charset = ANSI_CHARSET
            Font.Color = clBlack
            Font.Height = -16
            Font.Name = 'Arial Narrow'
            Font.Style = [fsBold]
            GradientEndColor = clWhite
            GradientStartColor = clFuchsia
            Options = [goVertLine, goHorzLine, goFixedHotTrack]
            ParentDoubleBuffered = False
            ParentFont = False
            ScrollBars = ssNone
            TabOrder = 1
            Visible = False
            OnDrawCell = stgEstadoProcesoDrawCell
          end
        end
      end
    end
  end
end
