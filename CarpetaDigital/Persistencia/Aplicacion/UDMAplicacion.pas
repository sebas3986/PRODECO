...
uses
  windows,  forms,  Sysutils,  Dialogs, strutils,  IOUtils,  Classes,  DBClient,
  FPGenerales, UDAOParametro,   UActualizaModulo, UGlobales;
...
type
  ...
  TDMAplicacion = class(TDataModule)
    ...
    public
     ...
     {METODOS}
      Procedure BorrarEjecutableAnterior(p_NombArch: string);
...
begin
  try
    try
      ApliModu  := ifthen (AmbiEjecApli= 'CLIE', 'la Aplicación', 'el Módulo') ;
      ...
      begin
        ...
        raise Exception.Create('No existe Información de Parámetros para ' + ApliModu +
                                ' : [' + p_DescModu + ']');
    except
      on e:Exception do
        raise Exception.Create('Error configurando Parámetros para ' + ApliModu + '. '
                             + #10#13 + '* ' + e.Message);
    ...
    begin
      ...
      begin
        ...
        if AmbiEjecApli = 'CLIE' then
          FRutaDestino  :=  ExtractFileDrive(Application.ExeName) + '\'
                            + LeftStr(FArchivoEjecutable,Pos('.exe',LowerCase(FArchivoEjecutable)) - 1)
                            + '\'
        else
        begin

          if MidStr(FRutaDestino,2,1) <> ':' then
          begin
            FRutaDestino  := ifThen(AnsiLeftStr(FRutaDestino,1) = '\',
                                                 FRutaDestino, '\' + FRutaDestino );
            if Pos(LowerCase(FRutaDestino),LowerCase(ExtractFileDir(Application.ExeName))) = 0  then
              FRutaDestino  := ExtractFileDir(Application.ExeName) + FRutaDestino
            else
              FRutaDestino  := ExtractFileDir(Application.ExeName);
          end;
        ...
        {LAS SIGUINTES LINEAS SON PARA ASEGURAR EL BORRADO DEL EJECUTABLE QUE ANTERIORMENTE
          ...
          DONDE SE INSTALO}
        ArchElim  := FRutaDestino + 'ActualizadorCarpeta.exe';
        BorrarEjecutableAnterior(ArchElim);
        ArchElim  := FRutaDestino + 'CarpetaDigital.ini';
        BorrarEjecutableAnterior(ArchElim);
        {--------------------------------------------------------------------}
...
{EL SIGUIENTE PROCEDIMIENTO ES TEMPORAL PARA ELIMINAR ARCHIVOS DE VERSIONES ANTERIORES.
  RECIBE COMO PARAMETRO EL ARCHIVO A BORRAR CON LA RUTA COMPLETA (UNIDAD Y CARPETA)}
Procedure TDMAplicacion.BorrarEjecutableAnterior(p_NombArch: string);
var
  ArchElim: string;
begin
  ArchElim  := p_NombArch;
  if FileExists(ArchElim) then
    TFile.Delete(ArchElim);
  ArchElim  := MidStr(ArchElim,3, 1000);
  if FileExists('c:' + ArchElim) then
    TFile.Delete('c:' + ArchElim);
  if FileExists('d:' + ArchElim) then
    TFile.Delete('d:' + ArchElim);
  if FileExists('c:\mtiexe\uniandesfabrica' + ArchElim) then
    TFile.Delete('c:\mtiexe\uniandesfabrica' + ArchElim);
  if FileExists('d:\mtiexe\uniandesfabrica' + ArchElim) then
    TFile.Delete('d:\mtiexe\uniandesfabrica' + ArchElim);
end;
