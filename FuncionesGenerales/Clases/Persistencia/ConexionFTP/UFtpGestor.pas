...
uses
  SysUtils,  strutils,  Classes,  IOUtils,  IdFTP,  IdAllFTPListParsers,   Vcl.Dialogs,
  IdFTPList;
...
type
  ...
  public
    ...
    function BorrarArchivos(P_RutaDire, P_NombArch: string): int32;
...
function TFtpGestor.BorrarArchivos(P_RutaDire, P_NombArch:string): Int32;
{P_NombArch DEBE INDICAR UN CONJUNTO. HACER USO DE CARACTERES COMODIN '*'}
var
  I         : Int32;
  DireOkok  : boolean;
begin
  try
    ...
    if DireOkok then
    begin
      FTP.List(P_NombArch,false);
      if FTP.DirectoryListing.Count > 2000 then
         ShowMessage('TIENE VALOR: ' + IntToStr(FTP.DirectoryListing.Count));
      for I := 0 to FTP.DirectoryListing.Count - 1 do
      begin
        FTP.Delete(FTP.DirectoryListing[I].FileName);
      end;
      if I > 2000 then
         ShowMessage('TIENE VALOR: ' + IntToStr(I));
