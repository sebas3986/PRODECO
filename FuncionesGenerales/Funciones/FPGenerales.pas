...
interface
uses
  Windows,  Vcl.Forms,System.Types,  controls,  IdSMTP,  IdMessage,  IdAttachmentFile,  SysUtils,
  StrUtils,  Classes,  StdCtrls,  IniFiles,  Mask,  ExtCtrls,  IdText,  Vcl.Graphics,
  ActnMenus, EnImgScr,  DateUtils,  EnDiGrph,  EnTransf,  EnBlowupForm,  RxRichEd,
  DBGrids, Variants;
  ...
  procedure OcultarOpcionesPrincipales (BarrMenu: TActionMainMenuBar);
  ...
  {DECLARACION DE FUNCIONES PARA MANEJO DEL OBJETO TDBGRID}
  procedure AjustarColumnasGrid(p_GridBase : TDBGrid);
...
{$REGION 'FUNCIONES PARA MANEJO DE MENUS'}
{ Funcion OcultarOpcionesPrincipales para que de um menú implementado en un objeto
          TActionMainMenuBarleer se oculten las opciones que posean TODAS sus opciones
          internas invisibles.
  Fecha Creación: 2017-05-19     Por: GUSTAVO VEGA ROZO
}
...
var
  I,J : Integer;
begin
  for I := 0 to (BarrMenu.ActionClient.Items.Count - 1) do
  begin
    J := 0;
    while(J < BarrMenu.ActionClient.Items[I].Items.Count)
            and not (BarrMenu.ActionClient.Items[I].Items[J].Action.Visible) do
      inc(J);
    if J >= BarrMenu.ActionClient.Items[I].Items.Count  then
      BarrMenu.ActionClient.Items[I].Visible:= False;
  end;
end;
...
begin
  ...
  try
    ...
    raise exception.create('No existe archivo de Configuración.');
  end;
end;
{$ENDREGION}

{$REGION 'FUNCIONES PARA CADENAS DE TEXTO' }
{ Funcion LlenarCerosIzquierda  Toma un valor numérico entero y devuelve una cadena
                                con el número y la cantidad de ceros a la izquerda
                                que se le indique.
  Fecha Creación: 2017-06-06    Por: GUSTAVO VEGA ROZO
  Parámetros: CantidadCeros [cadena que trae un valor numérico que indica la cantidad de
                              ceros a la izquierda que se van a colocar ]
              Valor [Valor entero que contiene el número original]
}
function LlenarCerosIzquierda(CantidadCeros:string; Valor:Integer):string;
begin
...
{ Funcion SoloNumeros  verifica que un caracter sea número únicamenta o backspace.
                       Se usa en conjunto con los eventos KeyPress para controlar
                       que en el objeto solamente se acepten números
  Fecha Creación: 2017-06-06     Por: GUSTAVO VEGA ROZO
  Parámetros: Valor [es el caracter que se va a evaluar. ]
}
function SoloNumeros(Valor:Char):Char;
begin
...
{$REGION 'FUNCIONES TDBGRID'}
procedure AjustarColumnasGrid(p_GridBase : TDBGrid);
var
  I         : Int32;
  MaxiAnch  : Int32; {MAXIMO ANCHO DE LA COLUMNA EN PIXELES}
  Canvas    : TCanvas;
begin
  Canvas:= TCanvas.Create;
  Canvas.Handle:= GetDC(0); {crea un nuevo manejador para el objeto canvas}
  for I := 0 to p_GridBase.Columns.Count - 1 do
  begin
    {ESTABLECE CANVAS CON EL TIPO DE LETRA DEL TITULO DE LA COLUMNA}
    Canvas.Font.Assign(p_GridBase.TitleFont);
    {COLOCA EL MAXIMO ANCHO IGUAL AL ANCHO DEL TITULO DE LA COLUMNA}
    MaxiAnch:= Canvas.TextWidth(p_GridBase.Columns[I].Title.Caption) + Canvas.TextWidth('W');
   {ESTABLECE CANVAS CON EL TIPO DE LETRA DEL GRID}
    Canvas.Font.Assign(p_GridBase.Font);
    with p_GridBase.DataSource.DataSet do
    begin
      First;
      DisableControls;
      while not eof do
      begin
        {SI EL ANCHO DEL TEXTO DEL DATO ES MAYOR AL MAXIMO LO CAMBIA}
        if (FieldByName(p_GridBase.Columns[I].FieldName).Value <> null)
            and (Canvas.TextWidth(FieldByName(p_GridBase.Columns[I].FieldName).Value) > MaxiAnch) then
          MaxiAnch := Canvas.TextWidth(FieldByName(p_GridBase.Columns[I].FieldName).Value)
                 + Canvas.TextWidth('W');
        Next;
      end;
      p_GridBase.Columns[i].Width := MaxiAnch;
      First;
      EnableControls;
    end;
  end;
