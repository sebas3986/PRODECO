...
uses
  ...
  Datasnap.DBClient, ZAbstractRODataset, ZAbstractDataset, ZDataset, frxClass, UGlobales,
...
type
  ...
  public
    ...
    procedure ActualizarCarpeta (idCarpeta : integer; ObraCivil, Insercion : boolean);
...
procedure TDMRecepcion.ActualizarCarpeta(idCarpeta: integer; ObraCivil, Insercion  : boolean);
begin
  try
       ...
       begin
         ...
         if NOT IsEmpty then
           begin
             ...
             if Criterio = 4 then   //Para imprmir el sticker de inserción
       ...
       begin
         ...
         if NOT IsEmpty then
           begin
             ...
             cdsSerieSubSerie.FieldByName('CodigoMCarpetaAleta').AsString := 'M'+Carpeta + LlenarCerosIzquierda('2', FieldByName('idSubSerieDocumental').AsInteger);  //Este es para la impresión del código de barras
             if Criterio = 0 then
               cdsSerieSubSerie.FieldByName('CodigoMCarpetaAletaI').AsString := 'M'+Carpeta + LlenarCerosIzquierda('2', FieldByName('idSubSerieDocumental').AsInteger)   //Para visualizar el texto de código de barras en creación
             else
               cdsSerieSubSerie.FieldByName('CodigoMCarpetaAletaI').AsString := 'M'+Carpeta + LlenarCerosIzquierda('2', FieldByName('idSubSerieDocumental').AsInteger)+' - I - '+IntToStr(cdsCarpeta.FieldByName('SecuenciaClase').AsInteger);   //Para visualizar el texto de código de barras en Inserción
             Result := True;
         end else
