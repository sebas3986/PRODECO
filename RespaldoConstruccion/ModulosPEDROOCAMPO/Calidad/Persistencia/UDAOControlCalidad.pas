...
uses
  System.SysUtils,  System.Classes,  UDMConexion,  Data.DB,  ZAbstractRODataset, UGlobales,
...
type
  TDAOControlCalidad = class(TDataModule)
    ...
    cdsCarpetaidCarpetaCrea: TIntegerField;
    ZQDatosCarpeta: TZQuery;
    dspDatosCarpeta: TDataSetProvider;
    dsDatosCarpeta: TDataSource;
    cdsDatosCarpeta: TClientDataSet;
    cdsDatosCarpetaidFlujo: TIntegerField;
    cdsDatosCarpetaidSerieDocumental: TIntegerField;
    cdsDatosCarpetaidCaja: TIntegerField;
    cdsDatosCarpetaidCarpeta: TIntegerField;
    cdsDatosCarpetaidTipoIdentificacion: TIntegerField;
    cdsDatosCarpetaCodigoCarpeta: TStringField;
    cdsDatosCarpetaNumeroDocumento: TStringField;
    cdsDatosCarpetaNombresApellidos: TStringField;
    cdsDatosCarpetaDescripcionTipoIdentificacion: TStringField;
    ZQReplicaData: TZQuery;
    ZQAletaFolio: TZQuery;
    dspAletaFolio: TDataSetProvider;
    cdsAletaFolio: TClientDataSet;
    dsAletaFolio: TDataSource;
    cdsAletaFolioidFolio: TIntegerField;
    cdsAletaFolioidSubSerieDocumental: TIntegerField;
    cdsAletaFolioCodigoFolio: TStringField;
  ...
  public
    ...
    procedure CambiarEstado (idCarpeta, Intentos : integer);
    function ValidarFolios (idCarpeta : integer) : boolean;
    procedure DatosCarpeta (idCarpeta : integer);
    function CarpetaNoCapturable (idCarpeta : integer) : boolean;
    procedure ReplicaData (idCarpeta : integer; Usuario : string);
...
procedure TDAOControlCalidad.AletaFolio(idFolio: integer);
begin
  try
    with ZQAletaFolio do
      begin
        Close;
        SQL.Clear;
        SQL.Add(Format('SELECT idFolio, SSD.idSubSerieDocumental, CodigoFolio, DescripcionSubSerieDocumental FROM %s.CarpetaAleta CA',[DMConexion.Esquema]));
        SQL.Add(Format('INNER JOIN %s.Folio FO ON (FO.idCarpetaAleta = CA.idCarpetaAleta)',[DMConexion.Esquema]));
        SQL.Add(Format('INNER JOIN %s.SerieSubSerie SSS ON (SSS.idSerieSubSerie = CA.idSerieSubSerie)',[DMConexion.Esquema]));
        ...
        if NOT IsEmpty then
          begin
            ...
            cdsAletaFolio.FieldByName('idFolio').AsInteger := FieldByName('idFolio').AsInteger;
            cdsAletaFolio.FieldByName('idSubSerieDocumental').AsInteger := FieldByName('idSubSerieDocumental').AsInteger;
            cdsAletaFolio.FieldByName('CodigoFolio').AsString := FieldByName('CodigoFolio').AsString;
            cdsAletaFolio.FieldByName('DescripcionSubSerieDocumental').AsString := FieldByName('DescripcionSubSerieDocumental').AsString;
        end;
    end;
  except
    on E : Exception do
      raise Exception.Create('Error TDAOControlCalidad.AletaFolio. FuenteError :' + E.Message);
  end;
end;
  ...
  try
    with ZQCarpeta do
       begin
         ...
         SQL.Add('DescripcionTipoSerieDocumental,DescripcionSubSerieDocumental, Foliada, CodigoFolio, idFolio, ChequeoCalidad, ClaseCarpeta, idCarpetaCrea');
         ...
         if NOT IsEmpty then
           begin
             ...
             repeat
               ...
               cdsCarpeta.FieldByName('idCarpetaCrea').AsInteger := FieldByName('idCarpetaCrea').AsInteger;
               Next;
             until Eof;
             Result := True;
         end else
...
procedure TDAOControlCalidad.CambiarEstado(idCarpeta, Intentos: integer);
var
   i : integer;
begin
  try
...
function TDAOControlCalidad.CarpetaNoCapturable(idCarpeta: integer): boolean;  //Verifica si la carpeta tiene folios capturables
begin
  Result := False;
  try
    ...
    on E : Exception do
      raise Exception.Create('Error TDAOControlCalidad.CarpetaCapturable. FuenteError : ' + E.Message);
  end;
end;

procedure TDAOControlCalidad.ChequeoCalidad(idFolio: integer; P_cheqfoli: Boolean; idFolioAnterior : integer);
{ESTA FUNCION CHEQUEA O DESCHEQUEA EL FOLIO Y DESBLOQUEA EL ANTERIOR. EL PARAMETRO P_cheqfoli INDICA SI SE CHEQUEA O DESCHEQUEA}
var
   Duplicado : boolean;
begin
  Duplicado := False;
  try
    with ZQChequeoCalidad do
       begin
         Close;
         SQL.Clear;
         SQL.Add(Format('UPDATE %s.Folio SET ChequeoCalidad = TRUE WHERE idFolio =:idFolio', [DMConexion.Esquema]));
         {SQL.Add(Format('UPDATE %s.Folio SET ChequeoCalidad = '+ ifthen(P_cheqfoli,'True','False'), [DMConexion.Esquema]));
         SQL.Add('       WHERE idFolio =:idFolio'); }
         ParamByName('idFolio').AsInteger := idFolio;
         ExecSQL;
    end;
...
procedure TDAOControlCalidad.DatosCarpeta(idCarpeta: integer);   //Obtiene los datos de la carpeta
begin
  try
    with ZQDatosCarpeta do
      begin
        Close;
        SQL.Clear;
        SQL.Add('SELECT DISTINCT idFlujo, idSerieDocumental, idCaja, CR.idCarpeta, ID.idTipoIdentificacion, CodigoCarpeta, NumeroDocumento,');
        SQL.Add('PrimerNombre||'' ''||SegundoNombre||'' ''||PrimerApellido||'' ''||SegundoApellido AS NombresApellidos, DescripcionTipoIdentificacion');
        SQL.Add(Format('FROM %s.Carpeta CR',[DMConexion.Esquema]));
        SQL.Add(Format('INNER JOIN %s.CarpetaAleta CA ON (CA.idCarpeta = CR.idCarpeta AND idSerieDocumental IN (3,4,8,9))',[DMConexion.Esquema]));
        SQL.Add(Format('INNER JOIN %s.Folio FO ON (FO.idCarpetaAleta = CA.idCarpetaAleta)',[DMConexion.Esquema]));
        ...
        SQL.Add(Format('INNER JOIN %s.TipoIdentificacion TI ON (TI.idTipoIdentificacion = ID.idTipoIdentificacion) WHERE CR.idCarpeta =:idCarpeta',[DMConexion.Esquema]));
        ParamByName('idCarpeta').AsInteger := idCarpeta;
        Open;
        cdsDatosCarpeta.Close;
        cdsDatosCarpeta.CreateDataSet;
        if NOT IsEmpty then
          begin
            First;
            repeat
              cdsDatosCarpeta.Append;
              cdsDatosCarpeta.FieldByName('idFlujo').AsInteger := FieldByName('idFlujo').AsInteger;
              cdsDatosCarpeta.FieldByName('idSerieDocumental').AsInteger := FieldByName('idSerieDocumental').AsInteger;
              cdsDatosCarpeta.FieldByName('idCaja').AsInteger := FieldByName('idCaja').AsInteger;
              cdsDatosCarpeta.FieldByName('idCarpeta').AsInteger := FieldByName('idCarpeta').AsInteger;
              cdsDatosCarpeta.FieldByName('idTipoIdentificacion').AsInteger := FieldByName('idTipoIdentificacion').AsInteger;
              cdsDatosCarpeta.FieldByName('CodigoCarpeta').AsString := FieldByName('CodigoCarpeta').AsString;
              cdsDatosCarpeta.FieldByName('NumeroDocumento').AsString := FieldByName('NumeroDocumento').AsString;
              cdsDatosCarpeta.FieldByName('NombresApellidos').AsString := FieldByName('NombresApellidos').AsString;
              cdsDatosCarpeta.FieldByName('DescripcionTipoIdentificacion').AsString := FieldByName('DescripcionTipoIdentificacion').AsString;
              Next;
            until Eof;
        end else
           cdsDatosCarpeta.EmptyDataSet;
    end;
  except
   on E : Exception do
     raise Exception.Create('Error TDAOControlCalidad.DatosCarpeta. FuenteError : ' + E.Message);
  end;
end;
  ...
  try
    with ZQImagenFolio do
       begin
         ...
         SQL.Add('VALUES (:idFolio, :RutaLocal, :RutaFTP, :NombreImagen, :ServidorFTP, :ipPublicacion, :TamanoBytes, :Ancho, :Alto, :Densidad, :Usuario)');
         ParamByName('idFolio').AsInteger := idFolio;
         ParamByName('TamanoBytes').AsInteger := TamanoBytes;
         ParamByName('Ancho').AsInteger := Ancho;
         ParamByName('Alto').AsInteger := Alto;
...
procedure TDAOControlCalidad.ReplicaData(idCarpeta: integer; Usuario: string);
begin
  try
    with ZQReplicaData do
      begin
        Close;
        SQL.Clear;
        SQL.Add(Format('SELECT ps_cantactu FROM %s.fnc_desplegaridentificacionfolios(:idCarpeta, :Usuario)',[DMConexion.Esquema]));
        ParamByName('idCarpeta').AsInteger := idCarpeta;
   ...
   on E : Exception do
     raise Exception.Create('Error TDAOControlCalidad.ReplicaData. FuenteError :' + E.Message);
  end;
end;
...
var
  FoliosCalidad, FoliosCarpeta : integer;
begin
  Result := False;
  try
    with ZQFoliosCalidad do
       begin
         Close;
         SQL.Clear;
         SQL.Add(Format('SELECT COUNT (idFolio) AS TotalFolios, CA.idCarpeta, ClaseCarpeta FROM %s.Carpeta CA',[DMConexion.Esquema]));
         SQL.Add(Format('INNER JOIN %s.CarpetaAleta CAL ON (CAL.idCarpeta = CA.idCarpeta)',[DMConexion.Esquema]));
         ...
         FoliosCalidad := FieldByName('TotalFolios').AsInteger; //Total folios a los que se les realizó calidad

         Close;
         SQL.Clear;
         SQL.Add(Format('SELECT COUNT (CantidadFolios) AS CantidadFolios, CA.idCarpeta, ClaseCarpeta FROM %s.Carpeta CA',[DMConexion.Esquema]));
         SQL.Add(Format('INNER JOIN %s.CarpetaAleta CAL ON (CAL.idCarpeta = CA.idCarpeta)',[DMConexion.Esquema]));
         SQL.Add(Format('INNER JOIN %s.Folio FO ON (FO.idCarpetaAleta = CAL.idCarpetaAleta) WHERE CA.idCarpeta =:idCarpeta AND TipoFolio <> ''T'' GROUP BY CAL.CantidadFolios, CA.idCarpeta',[DMConexion.Esquema]));
         ParamByName('idCarpeta').AsInteger := idCarpeta;
         Open;

         FoliosCarpeta := FieldByName('CantidadFolios').AsInteger; //Total folios de la carpeta

         if FieldByName('ClaseCarpeta').AsString = 'I' then
           Insercion := True
         else
           Insercion := False;

         if FoliosCalidad >= FoliosCarpeta then //Verifica si se cumplió la totalida de folios revisados en calidad
           Result := True;

         {if Result then   //Verifica si existen folios capturables para habilitar el flujo de captura (debe haber al menos un folio para captura)
           if CarpetaNoCapturable(idCarpeta) then
             Result := False;       }
    end;
  except
   on E : Exception do
