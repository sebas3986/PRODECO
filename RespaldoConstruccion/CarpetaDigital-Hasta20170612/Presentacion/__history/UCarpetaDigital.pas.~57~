...
uses
  ...
  RxCtrls,  RxExtenders,
  UGlobales, UFTP,  UProcesoCarpetaDigi,  UVisor, ULogin,  UProcesoLogin,
...
begin
  ...
  ProcCarp.VerificarVersion(NombModu, VersModu);
end;
...
begin
  ...
  if TipoUsuaConeProg <> 'CLIENTE' then
  begin
    if TipoUsuaConeProg = 'DESARROLLO' then
      PnlTitulo.Font.Color   := clPurple
    else
      if TipoUsuaConeProg = 'PRODUCCION' then
        PnlTitulo.Font.Color   := clBlue;
    PnlTitulo.Caption := PnlTitulo.Caption + '   [' + TipoUsuaConeProg + ']';
  end;
  TipoCapt    :='*';
  IndiBorrGrid:= True;
end;
