...
begin
  try
    ...
    begin
      ...
      {PARA LA ALETA FICTICIA "DOCUMENTOS DE IDENTIFICACION" SE CONCATENA I O N AL IDCARPETAALETA
       PARA PRODUCIR UN ORDENAMIENTO DIFERENTE AL DE LAS ALETAS. SALEN PRIMERO LOS FOLIOS CAPTURABLES
       DE LAS ALETAS DE INSERCION Y LUEGO LOS DE CREACION
      }
      SQL.Add('                           CASE WHEN TIAL.tipoaleta = ''D'' ');
      SQL.Add('                               THEN   CASE WHEN FOLI.tipofolio = ''R'' ');
      SQL.Add('                                             THEN ''I'' || CAST(FOLI.idcarpetaaletacrea AS VARCHAR) ');
      SQL.Add('                                             ELSE ''N'' || CAST(FOLI.idcarpetaaletacrea AS VARCHAR) ');
      SQL.Add('                                      END ');
      SQL.Add('                               ELSE CAST(FOLI.idcarpetaaletacrea AS VARCHAR) ');
...
begin
  try
    ...
    begin
      ...
      SQL.Add('                         CASE WHEN TIAL.tipoaleta = ''D'' ');
      SQL.Add('                               THEN   CASE WHEN ALPE.tipofolio = ''R'' ');
      SQL.Add('                                             THEN ''I'' || CAST(ALPE.idcarpetaaletacrea AS VARCHAR) ');
      SQL.Add('                                             ELSE ''N'' || CAST(ALPE.idcarpetaaletacrea AS VARCHAR) ');
      SQL.Add('                                      END ');
      SQL.Add('                               ELSE CAST(ALPE.idcarpetaaletacrea AS VARCHAR) ');
      SQL.Add('                         END idcarpetaaletacrea, ');
      ...
      SQL.Add('                                     DAPL.periodocotizacion, FOLI.codigofolio, FOLI.tipofolio, ');
      SQL.Add('                                     FOLI.idcarpetaaletacrea,FOLI.secuenciavisual, FOLI.capturable, ');
      SQL.Add('                                     IMAG.rutaftp, IMAG.nombreimagen');
      SQL.Add(Format('                        FROM %s.IDENTIFICACION IDEN',[DMConexion.esquema]));
      SQL.Add(Format('                        INNER JOIN %s.FOLIOIDENTIFICACION FOID ON '
                                              + 'FOID.ididentificacion = IDEN.ididentificacion',[DMConexion.esquema]));
