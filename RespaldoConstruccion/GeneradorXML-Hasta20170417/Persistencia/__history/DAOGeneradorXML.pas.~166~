...
type
  TDMGeneradorXML = class(TDataModule)
    ...
    cdsFoliosXMLidCarpeta: TIntegerField;
  private
  ...
  try
    with ZQAletasFolio do
       begin
         Close;
         SQL.Clear;
         SQL.Add(Format('SELECT CR.idCarpeta, DescripcionSubSerieDocumental FROM %s.Carpeta CR',[DMConexion.Esquema]));
         ...
         SQL.Add(Format('INNER JOIN andes_prod.SubSerieDocumental SSD ON (SSD.idSubSerieDocumental = SSS.idSubSerieDocumental) WHERE CR.idCarpeta =:idCarpeta',[DMConexion.Esquema]));
         ParamByName('idCarpeta').AsInteger := idCarpeta;
         Open;
...
procedure TDMGeneradorXML.BuscarCodigo(Codigo: string; Tipo : integer);    //Sirve para determinar la Caja, carpeta o Folio a partir del código que se ingresa
var
  ...
  try
    with ZQBuscarCodigo do
       begin
         ...
         case Tipo of
                 0 : begin
                       SQL.Add(Format('SELECT CJ.idCaja, idCarpeta, 0 AS idFolio FROM %s.Caja CJ',[DMConexion.Esquema]));
                       SQL.Add(Format('INNER JOIN %s.Carpeta CR ON (CR.idCaja = CJ.idCaja) WHERE CodigoCaja =:Codigo AND CJ.Habilitado = TRUE AND CR.Habilitado = TRUE',[DMConexion.Esquema]));
                     end;
                 1 : SQL.Add(Format('SELECT idCaja, idCarpeta, 0 AS idFolio FROM %s.Carpeta WHERE CodigoCarpeta =:Codigo AND Habilitado = TRUE',[DMConexion.Esquema]));
                 2 : begin
       ...
       begin
         ...
         case Tipo of
                ...
                2 : begin
                      ...
                      SQL.Add(Format('WHERE idFlujo IN (SELECT idFlujo FROM %s.Flujo WHERE idTareaProxima = 6 AND Firmado = FALSE) AND idFolio =:idFolio AND CA.Habilitado = TRUE ORDER BY 1',[DMConexion.Esquema]));
  ...
  try
    with ZQFolioIndexacion do
       begin
         ...
         SQL.Add(Format('INNER JOIN andes_prod.FolioIdentificacion FI ON (FI.idFolio = FO.idFolio) WHERE FO.idFolio =:idFolio',[DMConexion.Esquema]));
         ParamByName('idFolio').AsInteger := idFolio;
         Open;
  ...
  try
    with ZQFoliosXML do
       begin
         ...
         SQL.Add(Format('(SELECT SUM (CantidadFolios) AS CantidadFolios FROM %s.Carpeta CRA',[DMConexion.Esquema]));
         SQL.Add(Format('INNER JOIN %s.CarpetaAleta CL ON (CL.idCarpeta = CR.idCarpeta) WHERE CRA.idCarpeta = CL.idCarpeta GROUP BY CR.idCarpeta, CodigoCarpeta) AS CantidadFolios,',[DMConexion.Esquema]));
         SQL.Add('idFlujo, CR.idCarpeta, FO.idFolio, SecuenciaFolio, CodigoFolio, ipPublicacion, Densidad, FechaNomina, FO.Fecha AS FechaFolio, TamanoBytes, RutaFTP, NombreImagen, ObraCivil, Capturable');
         ...
         SQL.Add(Format('INNER JOIN %s.Folio FO ON (FO.idCarpetaAleta = CA.idCarpetaAleta AND Firmado = FALSE)',[DMConexion.Esquema]));
         ...
         SQL.Add('GROUP BY SD.idSerieDocumental, SSD.idSubSerieDocumental, CR.CodigoCarpeta, CR.idCarpeta, CA.CodigoCarpetaAleta, CA.CantidadFolios, CR.idFlujo, FO.idFolio, IM.ipPublicacion,');
         if idTipoSerieDocumental = 3 then
         ...
         if NOT IsEmpty then
           begin
             ...
             repeat
               ...
               cdsFoliosXML.FieldByName('idCarpeta').AsInteger := FieldByName('idCarpeta').AsInteger;
       ...
       begin
          ...
          Case idTipoSerieDocumental of
                     1 : begin
                           ...
                           SQL.Add(Format('INNER JOIN %s.CarpetaAleta CA ON (CA.idCarpetaAleta = FO.idCarpetaAleta)',[DMConexion.Esquema]));
                           SQL.Add(Format('INNER JOIN %s.SerieSubSerie SSS ON (SSS.idSerieSubSerie = CA.idSerieSubSerie)',[DMConexion.Esquema]));
                           SQL.Add(Format('INNER JOIN %s.SerieDocumental SD ON (SD.idSerieDocumental = SSS.idSerieDocumental AND idTipoSerieDocumental = 1)',[DMConexion.Esquema]));
                         end;
                     2 : SQL.Add(Format('SELECT NumeroDocumento, PrimerNombre, SegundoNombre, PrimerApellido, SegundoApellido, DescripcionTipoIdentificacion FROM %s.FolioIdentificacion FI',[DMConexion.Esquema]));
