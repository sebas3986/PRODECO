...
type
  TDAOCaptura = class
    ...
    public
      ...
      procedure CambiarEstadoCarpeta(p_IdenCarp: int32);
      ...
      Function VerificarCarpetaCompleta(p_IdenCarp: int32) : Boolean;
...
begin
  ...
  begin
    ...
    if IsEmpty then
      raise Exception.Create
                (Format('Imposible Actualizar información de Planilla para el Folio ID [%d].',
                        [p_IdenFoli]));
  end;
...
begin
  try
    ...
    on E:exception do
      raise Exception.Create
                (Format('No es posible agregar la Observación al Folio ID [%d].',[p_IdenFoli])
                + #10#13 + '* '+ e.Message);
  end;
end;
...
{ PROCEDIMIENTO PARA DISTRIBUIR LA PRIMER CAPTURA DE PERSONA DE LA CARPETA DE CREACION
  EN TODOS LOS FOLIOS NO CAPTURABLES DE LA CARPETA, BIEN SEA DE CREACION O DE INSERCION,
  PARA LA SERIE DOCUMENTAL DE HISTORIAS LABORALES.
...
begin
  try
    ...
    begin
      Close;
      SQL.Clear;
      {ESTA CONSULTA TRAE LA CANTIFAD DE FOLIOS NO CAPTURABLES PARA SABER SI DE DEBE
        REALIZAR DESPLIEGUE DE IDENTIFICACIONES O NO}
      SQL.Add('SELECT COUNT(*) foliosnocapturables ');
      ...
      begin
        ...
        begin
          Close;
          SQL.Clear;
          { SE EJECUTA FUNCION DE POSTGRES QUE REALIZA EL DESPLIEGUE DE IDENTIFICACIONES A
            LOS FOLIOS NO CAPTURABLES}
          SQL.Add('SELECT ps_cantactu');
          SQL.Add(Format('FROM %s.fnc_desplegaridentificacionfolios(%d,''%s'');',
                            [DMConexion.Esquema, p_idencarp,Paramstr(1)]));
          Open;
          First;
          if FieldByName('ps_cantactu').value <> FoliNoca then
...
begin
  try
    ...
    begin
      ...
      SQL.Add('       IMAG.nombreimagen, CARP.idcarpeta, CARP.codigocarpeta,  ');
      SQL.Add('       CARP.clasecarpeta, TARE.idtarea, ');
      SQL.Add('       CAJA.codigocaja, TISD.descripciontiposeriedocumental, ');
      SQL.Add('       SUBD.descripcionsubseriedocumental, FOAS.ps_diasbloq, FOAS.ps_horabloq, ');
      ...
      {LA SIGUIENTES TRES LINEAS SE USAN PARA QUE TRAIGA UN IDFOLIO ESPECIFICO, REEMPLAZAN LA
          LINEA DE ARRIBA}
      //SQL.Add(Format('FROM (SELECT idfolio as ps_idenfoli, 0 as ps_diasbloq, cast(''00:00:00'' as TIME) as ps_horabloq '
      //                        + ' FROM %s.folio WHERE CODIGOFOLIO = ''01090424170001N'') AS FOAS ' ,
      //                         [DMConexion.esquema]));

      SQL.Add(Format('INNER JOIN %s.FOLIO FOLI ON FOLI.idfolio = FOAS.ps_idenfoli',[DMConexion.esquema]));
      ...
      begin
        ...
        Result.DiasBloqueo        := FieldByName('ps_diasbloq').value;
        Result.HorasBloqueo       := FieldByName('ps_horabloq').value;
...
procedure TDAOCaptura.CambiarEstadoCarpeta(p_IdenCarp: int32);
begin
  try
    with FQuery do
    begin
      Close;
      SQL.Clear;
      {SE EJECUTA FUNCION DE BASE DE DATOS QUE CAMBIA EL FLUJO DE LA CARPETA SEGUN
       LA PARAMETRIA}
      SQL.Add('SELECT * ');
    ...
    on E:exception do
      raise Exception.Create
                (Format('No es posible cambiar el estado de la Carpeta ID [%d].',[p_IdenCarp])
                + #10#13 + '* '+ e.Message);
  end;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      on E:exception do
        raise Exception.Create
                  (Format('No es posible desbloquear el Folio ID [%d].',[p_IdenFoli])
                  + #10#13 + '* '+ e.Message);
  end;
end;

Function TDAOCaptura.VerificarCarpetaCompleta(p_IdenCarp: int32) : Boolean;
begin
  Result:= False;
  with FQuery do
  begin
