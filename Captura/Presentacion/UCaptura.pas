...
uses
  ...
  UCliente, UProcesoCaptura, UFtpGestor, UDMConexion,FPGenerales, UGlobales;
...
type
  TfrmCaptura = class(TForm)
    ...
    EdtTemporal: TEdit;
    LblSeleccioneSerie: TLabel;
    tmrSeleccionarSerie: TTimer;
    ...
    procedure redObservacionRegistroKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure dblSerieDocumentalDropDown(Sender: TObject);
    procedure dblSerieDocumentalCloseUp(Sender: TObject);
    procedure tmrSeleccionarSerieTimer(Sender: TObject);
...
var
  ...
  PuntMous  : TPoint;
  SaliModu  : Boolean = false; {CONTROLA LA SALIDA DEL MÓDULO}
  SelePers  : Boolean;
  TiemProc  : TTime;
  TipoCapt  : Char;
  VersModu  : string;
...
procedure TfrmCaptura.dblSerieDocumentalCloseUp(Sender: TObject);
begin
  if dblSerieDocumental.KeyValue >= 0 then
  begin
    spbNuevoFolio.Visible       := true;
    LblSeleccioneSerie.Visible  := False;
  end
  else
    tmrSeleccionarSerie.Enabled:= True;
end;

procedure TfrmCaptura.dblSerieDocumentalDropDown(Sender: TObject);
begin
  spbNuevoFolio.Visible     := False;
  LblSeleccioneSerie.Visible:= False;
end;
...
begin
  ...
  dblSerieDocumental.CloseUp(true);
...
begin
  SaliModu:=True;
  LblSeleccioneSerie.Visible:=False;
  if Application.MessageBox('Desea Salir del Módulo de Captura?', 'Advertencia',
...
begin
  ...
  begin
    ...
    begin
      ...
      {ACTUALIZACIÓN: [2016-03-14]
        SE REEMPLAZA LA SIGUIENTE LÍNEA PARA VALIDAR UNA DIFERENCIA DE HASTA DIEZ AÑOS EN LA FECHA DE
    ...
    begin
      Application.MessageBox(PChar('Fecha incorrecta. Se admiten fechas entre ['
                              ...
                              + ifthen(mkePeriodoCotizacion.Text = '  /    ', '.',
                                        ' (Hasta 10 años mas del Periodo de Cotización).')),
                              'Fecha Pago Banco',
                              MB_OK + MB_ICONERROR);
      bbtConfirmarDatos.Enabled := False;
    ...
    begin
      Application.MessageBox(PChar('Periodo incorrecto. Se admiten periodos entre ['
                              ...
                              + ifthen(mkeFechaPagoBanco.Text = '  /  /    ', '.',
                                        ' (Mínimo 10 años menos que la Fecha de Pago al Banco).')),
                              'Fecha Pago Banco',
                              MB_OK + MB_ICONERROR);
      bbtConfirmarDatos.Enabled := False;
...
begin
  if ProcCapt.DatosFolio.DescTipoSerieDocum = 'HISTORIAS LABORALES' then
    NotaAdic := #13#10 + #13#10
                +'NOTA: Recuerde que los Folios de HISTORIAS LABORALES admiten únicamente '
                +'un registro de Persona.'
                + #13#10 + 'El Folio se cerrará de una vez.'
  else
    NotaAdic := '';
  if chkObservacionRegistro.Checked  then
  begin
    if redObservacionRegistro.Text ='' then
      Application.MessageBox(PChar('No se permite guardar una Novedad de '
                              ...
                              'Captura de Información', MB_ICONINFORMATION + MB_OK)
    else
...
begin
  ...
  begin
    ZoomEmergente(frmLupa,imsFolio,'Función de Aumento (Lupa)',Self, 300,300,true);
    spbLupa.Caption:= 'Remover L&upa'
  end
  else
  begin
      ZoomEmergente(frmLupa,imsFolio,'Función de Aumento (Lupa)', Self,300,300,false);
  ...
  try
    ...
    begin
      Application.MessageBox(PChar ('No existen folios disponibles para Captura de la '
                             ...
                             'Captura de Información',MB_OK OR MB_ICONASTERISK);
      LimpiarControles;
    end;
  except
    on e:Exception do
...
procedure TfrmCaptura.tmrSeleccionarSerieTimer(Sender: TObject);
begin
  if SaliModu then
    LblSeleccioneSerie.Visible  := False
  else
    LblSeleccioneSerie.Visible  := true;
  tmrSeleccionarSerie.Enabled:= False;
end;
...
begin
  ...
  ProcCapt.VerificarVersion(NombModu, VersModu, VeriRuta);
  frmCaptura.Caption:= 'Captura de Información - [ Versión ' + VersModu + ' ]';
...
begin
  try
    ...
    on e:Exception do
    begin
       Application.MessageBox(PChar(e.Message), 'Captura de Información',MB_ICONWARNING + MB_OK);
       LimpiarDatosCaptura;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      If ProcCapt.DatosPersonasFolio.count > 0 then
        raise Exception.Create('El Folio tiene información de Captura y no registra '
                                + 'Fecha de Nómina .')
      else
      begin
    ...
    begin
      ...
      begin
        If ProcCapt.DatosPersonasFolio.count > 0 then
          raise Exception.Create('El Folio tiene información de Captura y no registra '
                                + 'información de la Entidad del Fondo, Período de '
                                + 'Cotización y Fecha de Pago al Banco.')
        else
        begin
          if ProcCapt.DatosFolio.NovedadesFolio.Count > 0 then
             raise Exception.Create('El Folio tiene Novedades de Captura y no registra '
